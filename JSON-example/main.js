...
app.post('/add', urlencodeParser, (reg, res) => {
    ...
    let all = jsonData.substring(0, jsonData.length-1)+', '+JSON.stringify(reg.body)+']';
    console.log(all);
...
app.get('/:name', (reg, res) => {
    if (reg.params.name === 'add') {
        res.render('add');
    } else if(reg.params.name === 'view'){
        res.render('vew', {data: jsObjectData});
    } else {
